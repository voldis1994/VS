...
let adminToken = import.meta.env.VITE_API_ADMIN_TOKEN || '';
let bootOnce: Promise<void> | null = null;

function ensureAdminToken(): Promise<void> {
  if (adminToken || bootOnce) return bootOnce || Promise.resolve();
  bootOnce = fetch('/api/v1/admin/lan-bootstrap')
    .then((r) => r.json())
    .then((j: { api_admin_token?: string }) => {
      if (j && j.api_admin_token) adminToken = j.api_admin_token;
    })
    .catch(() => undefined);
  return bootOnce;
}
...
export function apiFetch<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  return ensureAdminToken().then(() => {
    if (adminToken && !headers['x-admin-token'] && !headers['X-Admin-Token']) {
