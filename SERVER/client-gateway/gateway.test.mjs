...
describe('client gateway path policy', () => {
  ...
  it('never prints parenthesis placeholder as the phone URL', () => {
    const src = readFileSync(fileURLToPath(new URL('./gateway.mjs', import.meta.url)), 'utf8');
    expect(src).not.toContain('(set /etc/vs/client-url)');
    expect(src).toContain('tryServeClientStatic');
