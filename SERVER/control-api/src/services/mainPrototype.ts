 ...
 * Safety SL = 0.15% of price (0.00150). Best Outcome = SL to breakeven only.
 */
...
export const DESK_PROTOTYPE_STRATEGY = 'main-prototype-10s-sl015-be';

export function deskPrototypeRules(): string {
  return (
    `MAIN PROTOTYPE · closed 10s BUY/SELL · SL ${(SAFETY_SL_REL * 100).toFixed(2)}% (0.00150)` +
    ' · Best Outcome SL→BE after 0.15% plus · max 1 open · no flip every 10s candle'
  );
}
