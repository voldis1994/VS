 ...
 *
 * C++ vs-calc prepares EntryReady (direction + setup).
...
export function deskPrototypeRules(): string {
  return (
    `MAIN PROTOTYPE · C++ prepares entry · Node executes Capital · Entry SL ${(SAFETY_SL_REL * 100).toFixed(2)}% (${SAFETY_SL_REL.toFixed(4)})` +
