...
export function computeSafetyCushionStopLevel(
  ...
  const floor =
    abs >= 1000 ? SAFETY_SL_GOLD_FLOOR : abs >= 100 ? 0.5 : abs >= 10 ? 0.05 : 0.0005;
...
export function computeMarketProfitTrailStopLevel(
  ...
  // “Give back” cushion: keep room, but still follow fast enough.
  // ~0.07% on Gold (~4300) is ~3.0 points.
  const bufferPct = absEntry * 0.0007;
  ...
  if (direction === 'BUY') {
    ...
    if (minDist != null && minDist > 0) {
      ...
      const cushionedMinDist = minDist * 1.2;
