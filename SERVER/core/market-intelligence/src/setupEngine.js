...
export function evalCondition(name, pass, actual, threshold, detail) {
    return { name, status: pass ? 'PASS' : 'FAIL', actual, threshold, detail };
}
export const DEFAULT_TREND_CONTINUATION = {
...
export function evaluateTrendContinuationSetup(input) {
    ...
    conditions.push(evalCondition('market_state_ok', input.market.status === 'OK', input.market.status, 'OK', 'market measurements available'));
    conditions.push(evalCondition('feed_quality_ok', input.feed.quality === 'OK' || input.feed.quality === 'DEGRADED', input.feed.quality, 'OK|DEGRADED', input.feed.detail));
    conditions.push(evalCondition('trend_strength', ts != null && ts >= p.minTrendStrength, ts, p.minTrendStrength, 'normalized trend strength'));
    conditions.push(evalCondition('trend_quality', tq != null && tq >= p.minTrendQuality, tq, p.minTrendQuality, 'R² goodness-of-fit'));
    conditions.push(evalCondition('direction_magnitude', dir != null && Math.abs(dir) >= p.minAbsDirection, dir, p.minAbsDirection, 'abs(direction_score)'));
    conditions.push(evalCondition('noise_ceiling', noise == null || noise <= p.maxNoise, noise, p.maxNoise, '1 - R² noise score'));
    conditions.push(evalCondition('feed_confidence', feedConf == null || feedConf >= p.minFeedConfidence, feedConf, p.minFeedConfidence, 'multi-feed confidence'));
