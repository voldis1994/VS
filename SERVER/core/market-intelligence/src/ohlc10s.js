...
export function candle10sBucketStartMs(tsMs) {
    return Math.floor(tsMs / TEN_SEC_MS) * TEN_SEC_MS;
}
export function candle10sBucketIso(tsMs) {
...
export function emptyOhlc10sState(instrument) {
    return { instrument, bucket_start_ms: null, forming: null, closed: [] };
}
function seedCandle(instrument, tick, bucketStart) {
 ...
 */
export function ingestTickTo10s(state, tick) {
...
export function aggregateFrom10s(candles, periodSeconds) {
    if (periodSeconds < 10 || periodSeconds % 10 !== 0)
        return [];
...
export function candlesAvailableAt(candles, asOfIso) {
