 ...
 */
export function validateMultiFeed(input: ValidateFeedsInput): FeedValidationReport {
  ...
  const live = input.ticks.filter((t) => {
    if (t.instrument !== input.instrument) return false;
    if (!(t.bid > 0) || !(t.ask > 0) || t.ask < t.bid) return false;
    if (t.source_quality === 'ERROR') return false;
    return true;
  });
  ...
  const outlierScore =
    med != null && disp != null && disp > 0 && maxDev != null ? maxDev / disp : null;

  let quality: FeedValidationReport['quality'] = 'OK';
  let block: FeedValidationReport['block'] = null;
...
export function rawTickFromParts(input: {
  ...
  if (!(input.bid > 0) || !(input.ask > 0) || input.ask < input.bid) return null;
