...
export function updateExcursion(input) {
    const signed = input.direction === 'LONG' ? input.current - input.entry : input.entry - input.current;
    const peakFavorable = input.direction === 'LONG'
        ? Math.max(input.peak_price, input.current)
        : Math.min(input.peak_price, input.current);
    const peakProfit = input.direction === 'LONG' ? peakFavorable - input.entry : input.entry - peakFavorable;
...
export function rankExitCandidates(input) {
    ...
    candidates.push({
        action: 'HOLD',
        score: (input.momentum_score != null &&
