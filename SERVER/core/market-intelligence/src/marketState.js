...
function structurePattern(closes, highs, lows) {
    const sh = swingHighs(highs, 2);
    const sl = swingLows(lows, 2);
    if (sh.length < 2 || sl.length < 2)
        return null;
    const h1 = sh[sh.length - 2], h2 = sh[sh.length - 1];
    const l1 = sl[sl.length - 2], l2 = sl[sl.length - 1];
    const hh = h2 > h1;
    const hl = l2 > l1;
    const lh = h2 < h1;
    const ll = l2 < l1;
...
export function buildMarketStateVector(input) {
    ...
    const compression = avgRange && avgRange > 0 ? clamp(1 - lastRange / avgRange, 0, 1) : null;
    const expansion = avgRange && avgRange > 0 ? clamp(lastRange / avgRange - 1, 0, 1) : null;
    ...
    const structureScore = structure === 'HH_HL' ? 0.8 : structure === 'LH_LL' ? -0.8 : structure === 'MIXED' ? 0 : null;
    const breakout = atrVal && avgRange
        ? clamp(Math.abs(last - closes[closes.length - 6]) / Math.max(atrVal, 1e-9) - 1, 0, 1)
        : null;
    const noise = r2 == null ? null : clamp(1 - r2, 0, 1);
    const reversal = structureScore != null && momentum != null && Math.sign(structureScore) !== Math.sign(momentum)
