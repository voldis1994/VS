/**
 * VS-System candle bias + strict 10s SCALPING entry gate.
 * Scores in MASTER are heuristic 0..1 (VS-System used ~0..100 — minEdge scaled).
 */
...
/**
 * Strict SCALPING entry gate — kills "buy the falling knife" soft scores.
 *
 * Rules:
 ...
 *
 * MASTER scores are 0..1 → default minEdge 0.12 (VS-System used 12 on ~0..100).
 */
export function scalpStrictEntryAllowed(input: {
  ...
  buyScore: number;
  sellScore: number;
  /** Minimum buy−sell gap (default 0.12 on 0..1 scores). */
  minEdge?: number;
  /** Block BUY when last-5 net% below this; SELL when above -this (default 0.012%). */
  maxAdverseNetPct?: number;
}): { ok: boolean; skip?: string; reason?: string } {
  const { signal, tfBias, tfNetPct, microBias, buyScore, sellScore } = input;
  const minEdge = Number.isFinite(input.minEdge) ? Number(input.minEdge) : 0.12;
  const adverse = Number.isFinite(input.maxAdverseNetPct)
  ...
  const edge = signal === 'BUY' ? buyScore - sellScore : sellScore - buyScore;
  if (!(edge >= minEdge)) {
    return {
      ok: false,
      skip: 'scalp_weak_edge',
      reason: `Score edge ${edge.toFixed(3)} < ${minEdge} — wait clearer setup`,
    };
  }
...
/** Derive TF + micro bias from MASTER bars (1m structure + last slice as micro). */
export function candleBiasFromBars(bars: Bar[] | null | undefined): {
  tf: ReturnType<typeof evaluateCandleBiasFive>;
  micro: ReturnType<typeof evaluateCandleBiasFive>;
} {
  const list = bars ?? [];
  const tf = evaluateCandleBiasFive(list, { includeForming: true });
  // Micro: prefer last 5 including forming; same series when short history
  const micro = evaluateCandleBiasFive(list.slice(-6), { includeForming: true });
  return { tf, micro };
}
