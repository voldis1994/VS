  ...
  // 7) Recovery + MT4 + filters modules exist
  {
    ...
    const manageOnlyDayRollMtmApi =
      ...
      readFileSync(
        ...
        'manageOnlyTick seeds day_start_equity from MTM equity after UTC day roll'
      );
    const recoverDayRollMtmApi =
      runtimeBody.includes(
        'day_start_equity seeds from live equity (parity with manageOnly/full tick)'
      ) &&
      runtimeBody.includes(
        'rebuild equity BEFORE UTC day-roll so day_start_equity seeds'
      ) &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'recover seeds day_start_equity from journal equity after UTC day roll'
      );
    const recoverPaperCashBalanceApi =
      ...
      readFileSync(
        ...
        'recover syncs paper cash balance so open-book MTM day_start uses cash+UPL'
      );
    const hydrateOpenMtmDayRollApi =
      runtimeBody.includes(
      ...
      readFileSync(
        ...
        'hydrateBookFromDisk MTM seeds day_start from cash+UPL before UTC day roll'
      );
    const deferOpenDayRollSansQuoteApi =
      runtimeBody.includes('shouldDeferUtcDayRoll') &&
      runtimeBody.includes(
        'Opens without a live mark / Capital unproven: defer UTC day-roll'
      ) &&
      (runtimeBody.match(
        /Opens without a live mark \/ Capital unproven: defer UTC day-roll/g
      )?.length ?? 0) >= 2 &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'hydrateBookFromDisk defers UTC day-roll when opens exist without quote'
      );
    const deferOpenDayRollStaleQuoteApi =
      runtimeBody.includes('quoteProvenForOpenDayRoll') &&
      runtimeBody.includes(
      ...
      readFileSync(
        ...
        'hydrateBookFromDisk defers UTC day-roll when open-book quote is disk/stale'
      );
    const deferFullTickOpenDayRollApi =
      runtimeBody.includes('replayingDiskMark') &&
      runtimeBody.includes(
        'interval/desk replay of market_cache marks must not seal day_start_equity'
      ) &&
      runtimeBody.includes(
        'parity with manageOnly/hydrate/recover'
      ) &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'full tick defers UTC day-roll when replaying disk/stale open-book quote'
      );
    const deferKeepSealedDailyPnlApi =
      runtimeBody.includes(
        'Defer: keep closed PnL for the still-sealed day — wiping to pnlToday'
      ) &&
      (runtimeBody.match(
        /Defer: keep closed PnL for the still-sealed day/g
      )?.length ?? 0) >= 2 &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
    ...
    const deferCapitalUnprovenDayRollApi =
      runtimeBody.includes('shouldDeferUtcDayRoll') &&
      runtimeBody.includes(
      ...
      readFileSync(
        join(root, 'src/master/__tests__/capitalVenueStatus.test.ts'),
        'utf8'
      ).includes(
        'recover defers UTC day-roll while Capital account unproven'
      );
    const postRollTodayPnlApi =
      runtimeBody.includes('creditClosedDailyPnl') &&
      runtimeBody.includes('pendingCalendarDayClosedPnl') &&
      runtimeBody.includes(
        'Restore closes that landed during the defer window'
      ) &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'rollDailyPnl restores closes credited during deferred UTC day'
      );
    const deferPendingJournalRebuildApi =
      runtimeBody.includes(
        'Park calendar-today journal closes for post-roll restore'
      ) &&
      (runtimeBody.match(
        /Park calendar-today journal closes for post-roll restore/g
      )?.length ?? 0) >= 2 &&
      runtimeBody.includes(
      ...
      readFileSync(
        ...
        'hydrate rebuilds pending today closes so post-defer roll keeps them'
      );
    const utcDayRollDeferredStatusApi =
      ...
      masterPageBody.includes('utc_day_roll_deferred') &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'status reports utc_day_roll_deferred while open-book mark unproven'
      );
    const riskStaleDayPnlGateApi =
      readFileSync(join(root, 'src/master/risk.ts'), 'utf8').includes(
        'utc_day_roll_deferred'
      ) &&
      readFileSync(join(root, 'src/master/risk.ts'), 'utf8').includes(
        'Fail-closed while UTC day-roll is deferred'
      ) &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'evaluateRisk blocks entries while daily_pnl_day lags UTC today'
      );
    const riskDollarGatesClosedPnlApi =
      readFileSync(join(root, 'src/master/risk.ts'), 'utf8').includes(
        'profitDaily = Math.max(equityDaily, account.daily_pnl)'
      ) &&
      readFileSync(join(root, 'src/master/risk.ts'), 'utf8').includes(
        'lossDaily = Math.min(equityDaily, account.daily_pnl)'
      ) &&
      readFileSync(join(root, 'src/master/risk.ts'), 'utf8').includes(
      ...
      readFileSync(
        ...
        'dollar day gates fold closed daily_pnl after post-defer day_start reseed'
      );
    const whyUtcDayRollDeferredApi =
      runtimeBody.includes(
        'surface utc_day_roll_deferred on Why even before a risk tick'
      ) &&
      runtimeBody.includes('dailyPnlDayLagged') &&
      readFileSync(
        join(root, 'src/master/__tests__/dailyPnl.test.ts'),
        'utf8'
      ).includes(
        'status Why surfaces utc_day_roll_deferred while daily_pnl_day lags UTC today'
      );
    const flatDayLagRollStatusApi =
      runtimeBody.includes(
        'Roll-exec defer OR sealed-day lag (flat paper can lag without open-mark defer)'
      ) &&
      runtimeBody.includes(
      ...
      readFileSync(
        ...
        'status utc_day_roll_deferred when flat paper daily_pnl_day lags UTC today'
      );
    const capitalDayRollUnprovenCardApi =
      masterRouteBody.includes(
        "card('UTC day roll',s.capital_account_proven===false?'UNPROVEN':(s.utc_day_roll_deferred?'DEFERRED':'ok')"
      ) &&
      masterPageBody.includes(
      ...
      masterPageBody.includes(
        ...
        'Capital unproven must win over defer'
      ) &&
      masterPageBody.includes("'UNPROVEN'") &&
      masterPageBody.includes("'DEFERRED'");
    const livePathCapitalEnvIsolateApi =
      readFileSync(
        join(root, 'src/master/__tests__/master.livePath.test.ts'),
        'utf8'
      ).includes(
      ...
      readFileSync(
        join(root, 'src/master/__tests__/master.livePath.test.ts'),
        'utf8'
      ).includes('prevCapital') &&
      readFileSync(
        join(root, 'src/master/__tests__/master.livePath.test.ts'),
        'utf8'
      ).includes('delete process.env.CAPITAL_API_KEY');
    ...
    const honestyOk =
      ...
      manageOnlyDayRollMtmApi &&
      recoverDayRollMtmApi &&
      recoverPaperCashBalanceApi &&
      hydrateOpenMtmDayRollApi &&
      deferOpenDayRollSansQuoteApi &&
      deferOpenDayRollStaleQuoteApi &&
      deferFullTickOpenDayRollApi &&
      deferKeepSealedDailyPnlApi &&
      deferCapitalUnprovenDayRollApi &&
      postRollTodayPnlApi &&
      deferPendingJournalRebuildApi &&
      utcDayRollDeferredStatusApi &&
      riskStaleDayPnlGateApi &&
      riskDollarGatesClosedPnlApi &&
      whyUtcDayRollDeferredApi &&
      flatDayLagRollStatusApi &&
      capitalDayRollUnprovenCardApi &&
      livePathCapitalEnvIsolateApi &&
