...
describe('traderVision', () => {
  ...
  it('decideEntryFrom10sRegime returns null at spike top', () => {
    const { bars, top } = spikeTopScenario();
    expect(decideEntryFrom10sRegime(top, 'TRANSITION', bars)).toBeNull();
  });

  it('allows BUY on dip mid-range in uptrend', () => {
    const { bars, live } = dipBuyScenario();
    const view = buildTraderView(bars, live)!;
    expect(view.rangePos).toBeLessThan(0.72);
    const gate = traderEntryGate('BUY', view, live);
    expect(gate.ok).toBe(true);
    expect(gate.reason).toMatch(/OK BUY/);
  });

  it('allows entry on dip via decideEntryFrom10sRegime', () => {
    const { bars, live } = dipBuyScenario();
    const entry = decideEntryFrom10sRegime(live, 'TRANSITION', bars);
    expect(entry?.direction).toBe('BUY');
    expect(entry?.reason).toMatch(/TRADER|OK BUY/);
  });
});
