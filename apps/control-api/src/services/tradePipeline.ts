 ...
 * No fake success. Each stage returns a typed outcome; breaks surface as WAIT_* or ERROR_*.
 */
...
export function runTradePipeline(input: PipelineInput): PipelineResult {
  ...
  if (input.max_spread != null && n.spread > input.max_spread) {
    return {
      ok: false,
      stage: 'NORMALIZE',
      code: DecisionCodes.WAIT_SPREAD_TOO_HIGH,
  ...
  if (!entry) {
    ...
    return {
      ok: false,
      stage: 'SETUP',
      code: fade ? DecisionCodes.WAIT_NO_FADE : DecisionCodes.WAIT_NO_SETUP,
  ...
  if (!(input.lot_size > 0) || !Number.isFinite(input.lot_size)) {
    return {
      ok: false,
      stage: 'RISK',
      code: DecisionCodes.WAIT_RISK_REJECTED,
