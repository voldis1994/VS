...
function bar(o: number, h: number, l: number, c: number, i = 0): TenSecBar {
  return { open_time_ms: i * 10_000, open: o, high: h, low: l, close: c, ticks: 2 };
}
...
describe('multi-client same epic isolation', () => {
  ...
  it('two accounts on GOLD keep independent regime books', () => {
    ...
    const rangeBars = [
      bar(2000, 2000.2, 1999.8, 2000, 0),
      bar(2000, 2000.1, 1999.9, 2000.05, 1),
      bar(2000.05, 2000.15, 1999.95, 2000, 2),
    ];
    // Account 2: strong up trend-ish bodies
    const upBars = [
      bar(2000, 2004, 1999.5, 2003.5, 0),
      bar(2003.5, 2008, 2003, 2007.5, 1),
      bar(2007.5, 2012, 2007, 2011, 2),
      bar(2011, 2015, 2010.5, 2014, 3),
      bar(2014, 2018, 2013.5, 2017, 4),
      bar(2017, 2021, 2016.5, 2020, 5),
    ];
  ...
  it('local classifyRegime is independent of shared books', () => {
    const barsA = [bar(100, 100.1, 99.9, 100, 0), bar(100, 100.05, 99.95, 100.02, 1)];
    const barsB = [
      bar(100, 101, 99.5, 100.8, 0),
      bar(100.8, 102, 100.5, 101.7, 1),
      bar(101.7, 103, 101.4, 102.6, 2),
      bar(102.6, 104, 102.3, 103.5, 3),
      bar(103.5, 105, 103.2, 104.4, 4),
      bar(104.4, 106, 104.1, 105.3, 5),
    ];
