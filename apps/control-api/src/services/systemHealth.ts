...
export async function buildSystemHealth(opts?: {
  ...
  if (oldest != null) {
    if (oldest > 30_000) {
      marketLevel = 'ERROR';
      marketCode = DecisionCodes.WAIT_STALE_FEED;
  ...
  // RISK
  const riskReject = lastTick?.code === DecisionCodes.WAIT_RISK_REJECTED || lastTick?.code === DecisionCodes.RISK_REJECTED;
  subsystems.push({
    id: 'RISK',
    name: 'RISK',
    level: riskReject ? 'WARNING' : 'OK',
    code: riskReject ? String(lastTick?.code) : 'OK',
    detail: riskReject ? lastTick?.detail || 'Risk rejected' : 'Risk gates nominal (SL 0.20% of price)',
    broken_at: null,
    error_code: null,
    broker_error: null,
    retry_count: 0,
