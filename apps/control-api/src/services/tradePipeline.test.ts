...
describe('P3 tradePipeline', () => {
  it('STOP / trading off blocks with WAIT_TRADING_OFF', () => {
    const r = runTradePipeline(base({ stopped: true }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.code).toBe(DecisionCodes.WAIT_TRADING_OFF);
  });

  it('market closed blocks with WAIT_MARKET_CLOSED', () => {
    const r = runTradePipeline(base({ quote: quote({ market_status: 'CLOSED' }) }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.code).toBe(DecisionCodes.WAIT_MARKET_CLOSED);
  });

  it('stale feed blocks with WAIT_STALE_FEED', () => {
    const r = runTradePipeline(base({ feed_age_ms: 20_000 }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.code).toBe(DecisionCodes.WAIT_STALE_FEED);
  });
  ...
  it('spread too high blocks', () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.code).toBe(DecisionCodes.WAIT_SPREAD_TOO_HIGH);
  });
  ...
  it('invalid lot → WAIT_RISK_REJECTED', () => {
    const r = runTradePipeline(
      base({
        lot_size: 0,
    ...
    // lot 0 fails at risk if entry exists; if no entry, WAIT_NO_SETUP
    if (!r.ok && r.code === DecisionCodes.WAIT_NO_SETUP) {
      expect(r.code).toBe(DecisionCodes.WAIT_NO_SETUP);
    } else if (!r.ok) {
      expect(r.code).toBe(DecisionCodes.WAIT_RISK_REJECTED);
    } else {
