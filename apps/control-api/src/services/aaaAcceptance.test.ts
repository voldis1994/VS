...
describe('P6 systemHealth + P9 acceptance gates (mock-safe)', () => {
  ...
  it('P9: STOP / stale / market-closed / duplicate gates are real decision codes', async () => {
    ...
    if (!closed.ok) expect(closed.code).toBe(DecisionCodes.WAIT_MARKET_CLOSED);
    ...
    if (!stop.ok) expect(stop.code).toBe(DecisionCodes.WAIT_TRADING_OFF);
    ...
    if (!stale.ok) expect(stale.code).toBe(DecisionCodes.WAIT_STALE_FEED);
  });
});
