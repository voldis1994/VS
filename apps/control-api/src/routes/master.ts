...
async function refresh(){
  try{
    ...
    cards.innerHTML=[
      ...
      card('Daily PnL',s.capital_account_proven===false?'—':(s.account?.daily_pnl!=null?((cyclePending?'hydrated · ':'')+Number(s.account.daily_pnl).toFixed(2)):'—'),cyclePending&&s.capital_account_proven!==false&&s.account?.daily_pnl!=null?'warn':''),
      ...
      card('Day start eq',s.capital_account_proven===false?'UNPROVEN':(s.account?.day_start_equity!=null?((cyclePending?'hydrated · ':'')+Number(s.account.day_start_equity).toFixed(2)):'—'),s.capital_account_proven===false?'bad':(cyclePending&&s.account?.day_start_equity!=null?'warn':'')),
