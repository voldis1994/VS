...
async function refresh(){
  try{
    ...
    cards.innerHTML=[
      ...
      card('SETUP',s.market_setup?((s.setup_gate_armed?'gate · ':'')+s.market_setup.status+(s.market_setup.side?' '+s.market_setup.side:'')+' · '+s.market_setup.kind):'—',s.setup_gate_armed&&(!s.market_setup||s.market_setup.status!=='ARMED')?'bad':(!cyclePending&&s.market_setup&&s.market_setup.status==='ARMED'?'ok':(cyclePending&&s.market_setup?'warn':(s.market_setup&&String(s.market_setup.reason||'').indexOf('hydrated ·')===0?'warn':'')))),
