...
async function refresh(){
  try{
    ...
    cards.innerHTML=[
      ...
      card('Cycles by epic',(function(){var c=s.cycles_by_epic;if(!c)return'—';var keys=Object.keys(c).sort();if(!keys.length)return'—';var active=String(s.epic||'').trim().toUpperCase();return keys.map(function(epic){var row=c[epic]||{};var setup=row.market_setup;var setupBit=setup?((setup.status||'?')+(setup.side?(' '+setup.side):'')):'no-setup';var mark=active&&epic.toUpperCase()===active?'*':'';return mark+epic+':'+setupBit+' · '+(row.decision_kind||'—');}).join(' · ').slice(0,240);})(),s.cycles_by_epic&&Object.keys(s.cycles_by_epic).length>=2?'ok':(s.cycles_by_epic&&Object.keys(s.cycles_by_epic).length===1?'warn':'')),
