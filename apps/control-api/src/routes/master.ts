...
async function refresh(){
  try{
    ...
    cards.innerHTML=[
      ...
      card('Client fanout',s.last_client_fanout?((cyclePending?'hydrated · ':'')+(s.last_client_fanout.attempted?(s.last_client_fanout.detail||'—'):'idle')):'—',!cyclePending&&s.last_client_fanout&&s.last_client_fanout.attempted&&s.last_client_fanout.ok_count>0?'ok':(!cyclePending&&s.last_client_fanout&&s.last_client_fanout.attempted&&s.last_client_fanout.fail_count>0&&!s.last_client_fanout.ok_count?'bad':(cyclePending&&s.last_client_fanout?'warn':''))),
      ...
      card('Entries',s.entries_armed===false?((cyclePending?'hydrated · ':'')+'PAUSED'+(s.entries_pause_reason?' · '+s.entries_pause_reason:'')):((cyclePending?'hydrated · ':'')+'armed'),!cyclePending&&s.entries_armed===false?'bad':(!cyclePending?'ok':'warn')),
      card('SETUP',s.market_setup?((cyclePending?'hydrated · ':'')+(s.setup_gate_armed?'gate · ':'')+s.market_setup.status+(s.market_setup.side?' '+s.market_setup.side:'')+' · '+s.market_setup.kind):'—',!cyclePending&&s.setup_gate_armed&&(!s.market_setup||s.market_setup.status!=='ARMED')?'bad':(!cyclePending&&s.market_setup&&s.market_setup.status==='ARMED'?'ok':(cyclePending&&s.market_setup?'warn':(s.market_setup&&String(s.market_setup.reason||'').indexOf('hydrated ·')===0?'warn':'')))),
      card('Desk entry',s.desk_entry?((cyclePending?'hydrated · ':'')+s.desk_entry.source+' · '+s.desk_entry.side+(s.desk_entry.setup_kind?' · '+s.desk_entry.setup_kind:'')):'—',!cyclePending&&s.desk_entry?'ok':(cyclePending&&s.desk_entry?'warn':'')),
      card('Hour bias',s.hour_bias?((cyclePending?'hydrated · ':'')+s.hour_bias):'—',!cyclePending&&(s.hour_bias==='UP'||s.hour_bias==='DOWN')?'ok':(cyclePending&&s.hour_bias?'warn':(s.hour_bias==='UNKNOWN'?'warn':''))),
      card('Closed 10s',s.closed_10s_cached?'cached · present':(s.closed_10s_source==='journal'?((cyclePending?'hydrated · ':'')+'journal · present'):((cyclePending?'hydrated · ':'')+(s.closed_10s_present?'present':'none'))),!cyclePending&&s.closed_10s_present&&s.closed_10s_source!=='journal'&&!s.closed_10s_cached?'ok':'warn'),
      ...
      card('Hour bars',s.hour_bars_cached?('cached · '+(s.hour_bars_available||0)):(s.hour_bars_source==='live'?('live · '+(s.hour_bars_available||0)):(s.hour_bars_available?String(s.hour_bars_available):'—')),(s.hour_bars_available||0)>0&&(s.hour_bars_available||0)<6?'bad':(s.hour_bars_cached?'warn':((s.hour_bars_available||0)>=6?'ok':''))),
      ...
      card('Close fail',s.last_close_failed?((cyclePending?'hydrated · ':'')+(s.last_close_failed.exit_reason||'')+' · '+(s.last_close_failed.detail||'')).slice(0,80):'—',!cyclePending&&s.last_close_failed?'bad':(cyclePending&&s.last_close_failed?'warn':'')),
      ...
      card('Equity',s.capital_account_proven===false?'UNPROVEN':(s.account?.equity!=null?((cyclePending?'hydrated · ':'')+Number(s.account.equity).toFixed(2)):'—'),s.capital_account_proven===false?'bad':(cyclePending&&s.account?.equity!=null?'warn':'')),
      card('Available',s.capital_account_proven===false?'—':(s.account?.available_to_deal!=null?((cyclePending?'hydrated · ':'')+Number(s.account.available_to_deal).toFixed(2)):'—'),cyclePending&&s.capital_account_proven!==false&&s.account?.available_to_deal!=null?'warn':''),
      card('Trade allowed',s.account?.trade_allowed===false?((cyclePending?'hydrated · ':'')+'NO'):(s.account?.trade_allowed===true?((cyclePending?'hydrated · ':'')+'YES'):'—'),!cyclePending&&s.account?.trade_allowed===false?'bad':(!cyclePending&&s.account?.trade_allowed===true?'ok':(cyclePending&&s.account?.trade_allowed!=null?'warn':''))),
      card('News',s.news_window?.window_active?((cyclePending?'hydrated · ':'')+s.news_window.impact+' · '+s.news_window.source):(cyclePending&&s.news_window?'hydrated · clear':'clear'),!cyclePending&&s.news_window?.window_active&&s.news_window?.impact==='high'?'bad':(cyclePending&&s.news_window?'warn':'')),
      card('Daily PnL',s.capital_account_proven===false?'—':(s.account?.daily_pnl!=null?((s.utc_day_roll_deferred?'deferred · ':(cyclePending?'hydrated · ':''))+Number(s.account.daily_pnl).toFixed(2)):'—'),s.utc_day_roll_deferred?'warn':(cyclePending&&s.capital_account_proven!==false&&s.account?.daily_pnl!=null?'warn':'')),
      ...
      card('Confirm PnL',(function(){var rows=s.performance_by_desk_entry||[];if(!rows.length)return'—';var bits=rows.filter(function(r){return r.trades>0;}).map(function(r){return r.source+':'+r.trades+'/'+Number(r.total_pnl).toFixed(1);});var body=bits.length?bits.join(' · ').slice(0,80):'—';return body==='—'?body:((cyclePending?'hydrated · ':'')+body);})(),!cyclePending&&s.performance_by_desk_entry&&s.performance_by_desk_entry.some(function(r){return r.source!=='none'&&r.trades>0;})?'ok':(cyclePending&&s.performance_by_desk_entry&&s.performance_by_desk_entry.some(function(r){return r.trades>0;})?'warn':'')),
      card('Confirm EV',(function(){var rows=s.expectancy_by_desk_entry||[];if(!rows.length)return'—';var bits=rows.filter(function(r){return r.samples>0;}).map(function(r){return r.source+':'+r.samples+'/'+Number(r.avg_ev).toFixed(2);});var body=bits.length?bits.join(' · ').slice(0,80):'—';return body==='—'?body:((cyclePending?'hydrated · ':'')+body);})(),!cyclePending&&s.expectancy_by_desk_entry&&s.expectancy_by_desk_entry.some(function(r){return r.source!=='none'&&r.samples>0;})?'ok':(cyclePending&&s.expectancy_by_desk_entry&&s.expectancy_by_desk_entry.some(function(r){return r.samples>0;})?'warn':'')),
      card('Day start eq',s.capital_account_proven===false?'UNPROVEN':(s.account?.day_start_equity!=null?((s.utc_day_roll_deferred?'deferred · ':(cyclePending?'hydrated · ':''))+Number(s.account.day_start_equity).toFixed(2)):'—'),s.capital_account_proven===false?'bad':(s.utc_day_roll_deferred?'warn':(cyclePending&&s.account?.day_start_equity!=null?'warn':''))),
      card('UTC day roll',s.capital_account_proven===false?'UNPROVEN':(s.utc_day_roll_deferred?'DEFERRED':'ok'),s.capital_account_proven===false?'bad':(s.utc_day_roll_deferred?'warn':'ok')),
      card('Peak eq',s.capital_account_proven===false?'UNPROVEN':(s.account?.peak_equity!=null?((cyclePending?'hydrated · ':'')+Number(s.account.peak_equity).toFixed(2)):'—'),s.capital_account_proven===false?'bad':(cyclePending&&s.account?.peak_equity!=null?'warn':'')),
      card('Reject cool',(s.reject_cooldown_ms||0)>0?((cyclePending?'hydrated · ':'')+Math.ceil((s.reject_cooldown_ms||0)/1000)+'s'):'—',!cyclePending&&(s.reject_cooldown_ms||0)>0?'bad':(cyclePending&&(s.reject_cooldown_ms||0)>0?'warn':'')),
      card('Post-exit cool',(s.post_exit_cooldown_ms||0)>0?((cyclePending?'hydrated · ':'')+Math.ceil((s.post_exit_cooldown_ms||0)/1000)+'s'):'—',!cyclePending&&(s.post_exit_cooldown_ms||0)>0?'bad':(cyclePending&&(s.post_exit_cooldown_ms||0)>0?'warn':'')),
      ...
      card('Trades',s.traded!=null?((cyclePending?'hydrated · ':'')+String(s.traded)):'—',cyclePending&&s.traded!=null?'warn':''),
      card('Blocked',s.blocked!=null?((cyclePending?'hydrated · ':'')+String(s.blocked)):'—',cyclePending&&s.blocked!=null?'warn':''),
      card('Expectancy',s.performance?.trades?((cyclePending?'hydrated · ':'')+Number(s.performance.expectancy||0).toFixed(3)):'—',cyclePending&&s.performance?.trades?'warn':''),
      card('Fees',s.performance?.trades?((cyclePending?'hydrated · ':'')+Number(s.performance.total_fees||0).toFixed(2)):'—',cyclePending&&s.performance?.trades?'warn':''),
      card('Win rate',s.performance?.trades?((cyclePending?'hydrated · ':'')+(Number(s.performance.win_rate||0)*100).toFixed(1)+'%'):'—',cyclePending&&s.performance?.trades?'warn':''),
      card('Profit factor',s.performance?.trades&&s.performance?.profit_factor!=null&&Number.isFinite(s.performance.profit_factor)?((cyclePending?'hydrated · ':'')+Number(s.performance.profit_factor).toFixed(2)):'—',cyclePending&&s.performance?.trades&&s.performance?.profit_factor!=null?'warn':''),
      card('Loss streak',s.capital_account_proven===false?'—':(s.account?.consecutive_losses!=null?((cyclePending?'hydrated · ':'')+String(s.account.consecutive_losses)):'—'),!cyclePending&&s.capital_account_proven!==false&&(s.account?.consecutive_losses||0)>=3?'bad':(cyclePending&&s.capital_account_proven!==false&&s.account?.consecutive_losses!=null?'warn':'')),
      card('MC p50',s.monte_carlo?.equity_p50!=null?((cyclePending?'hydrated · ':'')+Number(s.monte_carlo.equity_p50).toFixed(2)):(s.monte_carlo?.p50!=null?((cyclePending?'hydrated · ':'')+Number(s.monte_carlo.p50).toFixed(2)):'—'),cyclePending&&(s.monte_carlo?.equity_p50!=null||s.monte_carlo?.p50!=null)?'warn':''),
      ...
      card('Max DD',s.performance?.trades?((cyclePending?'hydrated · ':'')+Number(s.performance.max_drawdown||0).toFixed(2)):'—',cyclePending&&s.performance?.trades?'warn':''),
