...
async function refresh(){
  try{
    ...
    cards.innerHTML=[
      card('Mode',s.mode),
      card('Epic',s.epic||'—'),
      card('Health',s.health,(s.health.includes('KILL')||s.health==='PERSIST_DEGRADED'||s.health==='LIVE_NO_CAPITAL'||s.health==='LIVE_UNATTACHED'||s.health==='LIVE_ACCOUNT_UNPROVEN'||s.health==='LIVE_QUOTE_STALE')?'bad':'ok'),
