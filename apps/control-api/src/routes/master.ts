...
async function refresh(){
  try{
    ...
    cards.innerHTML=[
      ...
      card('Confirm PnL',(function(){var rows=s.performance_by_desk_entry||[];if(!rows.length)return'—';var bits=rows.filter(function(r){return r.trades>0;}).map(function(r){return r.source+':'+r.trades+'/'+Number(r.total_pnl).toFixed(1);});return bits.length?bits.join(' · ').slice(0,80):'—';})(),s.performance_by_desk_entry&&s.performance_by_desk_entry.some(function(r){return r.source!=='none'&&r.trades>0;})?'ok':''),
      card('Confirm EV',(function(){var rows=s.expectancy_by_desk_entry||[];if(!rows.length)return'—';var bits=rows.filter(function(r){return r.samples>0;}).map(function(r){return r.source+':'+r.samples+'/'+Number(r.avg_ev).toFixed(2);});return bits.length?bits.join(' · ').slice(0,80):'—';})(),s.expectancy_by_desk_entry&&s.expectancy_by_desk_entry.some(function(r){return r.source!=='none'&&r.samples>0;})?'ok':''),
